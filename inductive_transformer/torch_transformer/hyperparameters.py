...
class HyperParameters:
    def __init__(
        ...
        self.perturbation_test_encoder_attention = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "encoder_attention"
        ]
        self.perturbation_test_encoder_token = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "encoder_token"
        ]
        self.perturbation_test_encoder_position = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "encoder_position"
        ]
        self.perturbation_test_decoder_attention = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "decoder_attention"
        ]
        self.perturbation_test_decoder_token = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "decoder_token"
        ]
        self.perturbation_test_decoder_position = PERTURBATION_TEST_WEIGHTS_TO_LEARN[
            "decoder_position"
        ]
        ...
        self.encoder_attention_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.layer_width, self.layer_width)
        self.encoder_token_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.vocab_size, self.layer_width)
        self.encoder_position_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.num_positions, self.vocab_size, self.layer_width)
        self.decoder_attention_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.layer_width, self.layer_width)
        self.decoder_token_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.vocab_size, self.layer_width)
        self.decoder_position_pi_weights: Optional[
            Tensor
        ] = None  # torch.ones(self.num_positions, self.vocab_size, self.layer_width)
    ...
    def construct_some_test_weights(self, all_weights_override: bool = False):
        ...
        if sentences == "small dog. big cat.":
            if self.perturbation_test_encoder_token or all_weights_override:
                self.encoder_token_pi_weights = torch.full(
                    (
                        self.num_layers,
                        self.num_positions,
                        self.vocab_size,
                ...
                self.encoder_token_pi_weights[0][0][3][
                    0
                ] = self.strong  # small in layer 0, left column
                self.encoder_token_pi_weights[1][1][4][
                    0
                ] = self.strong  # dog in layer 1, left column
                self.encoder_token_pi_weights[0][0][0][
                    1
                ] = self.strong  # big in layer 0, right column
                self.encoder_token_pi_weights[1][1][1][
                    1
                ] = self.strong  # cat in layer 1, right column
            if self.perturbation_test_decoder_token or all_weights_override:
                self.decoder_token_pi_weights = torch.full(
                    (
                        self.num_layers,
                        self.num_positions,
                        self.vocab_size,
                ...
                self.decoder_token_pi_weights[0][0][3][
                    0
                ] = self.strong  # small in layer 0, left column
                self.decoder_token_pi_weights[1][1][4][
                    0
                ] = self.strong  # dog in layer 1, left column
                self.decoder_token_pi_weights[0][0][0][
                    1
                ] = self.strong  # big in layer 0, right column
                self.decoder_token_pi_weights[1][1][1][
                    1
                ] = self.strong  # cat in layer 1, right column
            if self.perturbation_test_encoder_position or all_weights_override:
                self.encoder_position_pi_weights = torch.full(
                    (self.num_layers, self.num_positions, self.layer_width), self.weak
                )
                # Set the position weights to diagonal activations in both layers of the encoder
                self.encoder_position_pi_weights[0][0][
                    0
                ] = self.strong  # small in layer 0, position 0, left column
                self.encoder_position_pi_weights[1][1][
                    0
                ] = self.strong  # dog in layer 1, position 1, left column
            ...
            if self.perturbation_test_decoder_position or all_weights_override:
                self.decoder_position_pi_weights = torch.full(
                    (self.num_layers, self.num_positions, self.layer_width), self.weak
                )
                # Set the position weights to diagonal activations in both layers of the decoder
                self.decoder_position_pi_weights[0][0][
                    0
                ] = self.strong  # small in layer 0, position 0, left column
                self.decoder_position_pi_weights[1][1][
                    0
                ] = self.strong  # dog in layer 1, position 1, left column
            ...
            if self.perturbation_test_encoder_attention or all_weights_override:
                self.encoder_attention_pi_weights = torch.full(
                    (self.num_layers, self.layer_width, self.layer_width), self.weak
                )
            ...
            if self.perturbation_test_decoder_attention or all_weights_override:
                self.decoder_attention_pi_weights = torch.full(
                    (self.num_layers, self.layer_width, self.layer_width), self.weak
                )
                # Set the layer_1 attention weights to 0.5
                self.decoder_attention_pi_weights[1][0][0] = self.strong / 2
                self.decoder_attention_pi_weights[1][0][1] = self.strong / 2
                self.decoder_attention_pi_weights[1][1][0] = self.strong / 2
                self.decoder_attention_pi_weights[1][1][1] = self.strong / 2
                # Set the layer_0 straight connection weights to strong and leave the cross connection weights at 0
