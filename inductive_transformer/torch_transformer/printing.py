...
def format_prob_vocab(prob_outputs, vocab):
    ...
    try:
        ...
        prob_token_pairs = [
            (round(x.item() * 100) / 100, v) for x, v in zip(prob_outputs, vocab)
        ]
...
def print_to_terminal(
    model, iter, epoch, start, loss_avg, toc, print_every, total_epochs
):
    print(
...
def send_to_google_sheet(
    ...
    encoder_attention_pi_weights = torch.stack(
        [
            normalize_weights(
                model.encoder_layer_0.encoder_attention_pi.weights, dim=0
            ),
            normalize_weights(
                model.encoder_layer_1.encoder_attention_pi.weights, dim=0
            ),
        ],
        dim=0,
    )
    ...
    decoder_attention_pi_weights = torch.stack(
        [
            normalize_weights(
                model.decoder_layer_0.decoder_attention_pi.weights, dim=0
            ),
            normalize_weights(
                model.decoder_layer_1.decoder_attention_pi.weights, dim=0
            ),
        ],
        dim=0,
    )
    ...
    for sheet_number, prompt_tensor in enumerate(prompt_tensors):
        ...
        y = (
            model.decoder_pre_output_details
        )  # Actually use the decoder_pre_output_details
        if sheet_number == 0:
    ...
    attention_preds = [
        torch.empty(attention_input.shape) for _ in range(len(prompt_tensors))
    ]
    attention_truths = [
        torch.empty(attention_input.shape) for _ in range(len(prompt_tensors))
    ]
    ...
    res_pred_truth_input = format_into_pred_truth_table(
        ...
        preds=prompt_preds,
        truths=[truths[i] for i in range(len(prompt_tensors))]
    ...
    attention_preds = (
        [torch.empty(attention_input.shape) for _ in range(len(preds))]
    ...
    attention_inputs = (
        [attention_input for _ in range(len(preds))] if preds is not None else None
    )
...
def format_into_pred_truth_table(
    model,
    vocab,
    model_outputs,
    preds,
    ...
    token_start_index = 1
    attention_start_index = (
        1 + model.hyperparams.layer_width * model.hyperparams.num_positions
    )
    num_cols = (
        1
        + model.hyperparams.layer_width * model.hyperparams.num_positions
    ...
    for k in range(num_sentences):
        ...
        for n in range(model.hyperparams.num_layers):
            for lw in range(model.hyperparams.layer_width):
                row[
                    token_start_index + lw * model.hyperparams.num_positions
                ] = f"layer_width {lw}"
            ...
            def add_output_to_row(token_output, attention_output, label):
                ...
                for lw in range(model.hyperparams.layer_width):
                    for p in range(model.hyperparams.num_positions):
                        token_index = (
                            token_start_index + lw * model.hyperparams.num_positions + p
                        )
                        attention_index = attention_start_index + lw
                        row[token_index] = format_prob_vocab(
                            token_output[k][n][p, :, lw], vocab
                        )
                        if (
                            p == 0
                        ):  # The attention not split by position, so we just print it once
                            row[attention_index] = format_attention_tensor(
                                attention_output[k][:, lw]
                            )
                table.append(row)
                row = [None] * num_cols
...
def format_into_table(
    ...
    if output.shape == (
        model.hyperparams.num_layers,
        model.hyperparams.vocab_size,
        model.hyperparams.layer_width,
    ):
        return format_into_table_no_position(
            output, model, vocab, top_row, top_row_label, decoder
        )
    ############################
    ...
    for n in range(model.hyperparams.num_layers):
        ...
        result.append(
            [f"layer number: {n}"]
            + [
                f"{top_row_label} = {['%.2f' % l.item() for l in top_row[n][:, lw]]}"
                for lw in range(model.hyperparams.layer_width)
            ]
        )
        ...
        result.append(
            [None]
            + [
                f"position weights = {['%.2f' % p.item() for p in position_row[n][:, lw]]}"
                for lw in range(model.hyperparams.layer_width)
            ]
        )
        for p in range(model.hyperparams.num_positions):
            result.append(
                [f"layer number: {n}, position: {p}"]
                + [
                    format_prob_vocab(output[n, p, :, lw], vocab)
                    for lw in range(model.hyperparams.layer_width)
                ]
            )
    ...
    if decoder:
        result.append(
            ["Read bottom to top"]
            + [None for _ in range(model.hyperparams.layer_width)]
        )
    else:
        result.append(
            ["Read top to bottom"]
...
def format_into_table_no_position(
    output, model, vocab, top_row, top_row_label: str, decoder: bool = False
):
    assert output.shape == (
        model.hyperparams.num_layers,
        model.hyperparams.vocab_size,
    ...
    # Takes in the inference output of a model and formats it into a table
    result: List = [
        [None for _ in range(model.hyperparams.layer_width + 1)]
        for _ in range(model.hyperparams.num_layers * 4)
    ]
    for n in range(model.hyperparams.num_layers):
