    ...
    for epoch in range(epochs):
        for i in range(n_batches):
            ...
            if (i + 1) % print_every == 0:
                ...
                printing.print_to_terminal(model, i, epoch, start, loss_avg, toc, print_every)
                toc = time.time()
                total_loss = 0
            ...
            if output_to_google_sheet:
                models_parameters.append(
                    {
                        ...
                        "logical_decoder.input": copy.deepcopy(word_log_prob_tensors),
                ...
                if len(losses) > 1 and losses[-1] > losses[-2] and not reached_local_minimum:
                    ...
                    if prompt_tensors is not None:
                        ...
                        print_to_terminal(model, i, epoch, start, loss_avg, toc, print_every)
                        send_to_google_sheet(
                            prompt_tensors,
                            preds,
                            truths,
                            word_log_prob_tensors,
                            model,
                            models_parameters,
                            attention_input,
                            vocab,
                            z_decode,
                            i,  # FIXME: Should this be minimum_index?
                        )
